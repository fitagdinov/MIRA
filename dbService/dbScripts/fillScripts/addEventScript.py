...
def addEventScript(external_id_event: List[int],
    ...
    insert_query = \
        f"""
        INSERT IGNORE INTO NoOldMen.StaticEvent (EXTERNAL_ID_event, CITE_ID_event, 
        event_short_name, event_detailed_info, event_level_1, event_level_2, event_level_3)
        VALUES
        """
    for i in range(len(external_id_event)):
        insert_query += f"({external_id_event[i]}, '{cite_id_event[i]}', '{event_short_name[i]}', " \
