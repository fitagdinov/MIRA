...
class RequestGetEventByIdList(Schema):
    event_id_list = fields.List(fields.Integer(required=True, description="ID мероприятия"), required=True)


class ResponseGetEventByIdList(Schema):
...
class GetEventByIdList(MethodResource, Resource):
    @marshal_with(ResponseGetEventByIdList)
    @use_kwargs(RequestGetEventByIdList, location='query')
    def get(self, event_id_list: list):
        linked_groups_list = []
        scrap_all_events = get_request(query=f"SELECT * FROM StaticEvent WHERE SYS_ID_event in {tuple(event_id_list)}", execute_many=True)
